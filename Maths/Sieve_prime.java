...
public class Sieve_prime {
    ...
    public static void seive(int n, boolean[] prime) {
        for (int i = 2; i <= Math.sqrt(n); i++) { // only check the n squer here 20 = 4.47 something 4*4
            if (!prime[i]) { // is number is false it measn number is prime
                for (int j = i * 2; j <= n; j += i) { // if the 2 is prime so all the double of 2 is not prime like
                                                      // 4,6,8,10,12..
